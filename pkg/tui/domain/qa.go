...
// GetStagesSnapshot returns a read-locked copy of stages for rendering.
func (qt *QATracker) GetStagesSnapshot() (string, []*CheckpointStage, bool) {
	qt.mu.RLock()
	defer qt.mu.RUnlock()

	stages := make([]*CheckpointStage, len(qt.Stages))
	for i, s := range qt.Stages {
		cp := *s
		if len(s.Results) > 0 {
			cp.Results = make([]TestResult, len(s.Results))
			copy(cp.Results, s.Results)
		}
		stages[i] = &cp
	}
	return qt.Feature, stages, qt.WaitingApproval
}
